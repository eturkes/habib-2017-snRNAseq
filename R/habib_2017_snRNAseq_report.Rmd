...
```{r}
packages <- c(
  "conflicted", "BiocFileCache", "data.table", "DT", "Matrix", "SingleCellExperiment",
...
# Specifically in clusters 15-18 and they do not match those in the original publication.
# Therefore we use Supplementary Table 7 (nmeth.4407-S10.xlsx), which has consistent labels.
# As this file contained non-standard formatting, minimal manual editing was done for use with R.
...
edat <- scale(edat)
rm(dec, dec1, df_redDim, tmp_df, runTSNE)
```
...
We apply a k-means method atop the 50 PCs generated earlier, choosing to generate 15-18 clusters.

```{r}
all_num_clust <- c(15:18)
...
```{r}
all_vars <- c("habib_cluster_name", "habib_cluster", paste0("km_", all_num_clust, "_clusters"))
```
...
for (var in all_vars) {
  print(dim_red_plot(
    data = df_hvg, x = "hvg_tsne1", y = "hvg_tsne2", col = var, type = "cat"))}
```
...
for (var in all_vars) {
  print(dim_red_plot(
    data = df_hvg, x = "habib_tsne1", y = "habib_tsne2", col = var, type = "cat"))}
```
