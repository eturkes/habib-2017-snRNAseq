...
In order to remove droplets which do not contain a cell but are rather ambient RNA, we first visualize the inflection point on a knee plot, as described originally in @macosko_highly_2015.
Then we remove them using `emptyDrops` from the `DropletUtils` package.
...
# This chunk was run with "cache = TRUE".
set.seed(100)
emptyDrops_out <- emptyDrops(counts(sce))
saveRDS(emptyDrops_out, file = file.path(data_dir, "data", "emptyDrops_out.rds"))
```

```{r}
tapply(emptyDrops_out$Total, emptyDrops_out$FDR, summary)
```
