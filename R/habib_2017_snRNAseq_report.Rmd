...
par(mfrow = c(1, 1))
plot(
  log10(attr(reducedDim(sce), "percentVar")), xlab = "PC", ylab = "log10(Prop of variance explained)",
...
As an alternative method, we also cluster using SC3 [@kiselev_sc3_2017].
We SC3 to cluster the first 500 cells and then run an SVM to predict the labels of all other cells.
...
```{r}
all_vars = c("Cell_Type", paste0("km", all_num_clust), paste0("sc3_", all_ks, "_clusters"))
