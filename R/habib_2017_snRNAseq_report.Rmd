...
```{r, echo = FALSE, message = FALSE}
label_df <- data.frame(
  km_17_clusters_name = levels(df_hvg$km_17_clusters_name), label = levels(df_hvg$km_17_clusters_name))
label_df2 <- df_hvg %>%
  group_by(km_17_clusters_name) %>%
  summarize(hvg_tsne1 = median(hvg_tsne1), hvg_tsne2 = median(hvg_tsne2)) %>%
  left_join(label_df)
dim_red_plot(
  data = df_hvg, x = "hvg_tsne1", y = "hvg_tsne2", col = "km_17_clusters_name", type = "cat") +
...
kable(table(colData(sce)$habib_cluster, keep), row.names = TRUE) %>%
...
kable(table(colData(sce)$habib_cluster, except_ribo_keep), row.names = TRUE) %>%
  kable_styling(full_width = FALSE, position = "left")
```
...
names(tmp_df) <- paste0("allgenes_pc", seq(ncol(tmp_df)))
...
label_df2 <- df_allgenes %>%
  ...
  left_join(label_df)
dim_red_plot(
  data = df_allgenes, x = "habib_tsne1", y = "habib_tsne2", col = "habib_cluster_name", type = "cat") +
  xlab("tSNE 1") + ylab("tSNE 2") + ggtitle("Habib Clusters Habib tSNE")
dim_red_plot(
  data = df_allgenes, x = "habib_tsne1", y = "habib_tsne2", col = "cell_id_stem", type = "other") +
...
We first try replicating the tSNE in @habib_massively_2017 using all genes in the set, setting the perplexity to 100 as was done in the paper.
...
if (file.exists(rds)) {
  ...
  runTSNE <- runTSNE(sce, use_dimred = "PCA", perplexity = 100, rand_seed = 100)
  saveRDS(runTSNE, rds)}
```
...
label_df2 <- df_allgenes %>%
  ...
  left_join(label_df)
dim_red_plot(
  data = df_allgenes, x = "allgenes_tsne1", y = "allgenes_tsne2",
...
kable(table(dec$FDR < 1e-10, dec$bio > 0.014), row.names = TRUE) %>%
  kable_styling(full_width = FALSE, position = "left")
par(mfrow = c(1, 2))
...
tmp_df <- data.frame(reducedDim(sce_hvg, "PCA"))
...
```{r}
reducedDims(sce_hvg) <- SimpleList(TSNE = Rtsne$Y)
tmp_df <- data.frame(reducedDim(sce_hvg, "TSNE"))
names(tmp_df) <- paste0("hvg_tsne", seq(ncol(tmp_df)))
...
label_df <- data.frame(
  habib_cluster_name = levels(df_hvg$habib_cluster_name),
  label = levels(df_hvg$habib_cluster_name))
label_df2 <- df_hvg %>%
  group_by(habib_cluster_name) %>%
  summarize(hvg_tsne1 = median(hvg_tsne1), hvg_tsne2 = median(hvg_tsne2)) %>%
  left_join(label_df)
...
dim_red_plot(
  data = df_hvg, x = "hvg_tsne1", y = "hvg_tsne2", col = "cell_id_stem", type = "other") +
...
We see that the HVG tSNE is an improvement over the tSNE using all genes, so we use this tSNE for the analysis going forward.
...
We assign labels for the k-means clusters to the best of our ability based on their proximity with the clusters in @habib_massively_2017 and add them to the `sce_hvg` object.
...
colData(sce_hvg) <- cbind(
  colData(sce_hvg), km_17_clusters = df_hvg$km_17_clusters,
  km_17_clusters_name = df_hvg$km_17_clusters_name)
df_hvg$km_17_clusters_name <- factor(df_hvg$km_17_clusters_name)
sce_hvg
```
