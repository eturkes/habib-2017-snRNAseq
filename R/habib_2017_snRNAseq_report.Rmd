...
regions <- "*"
cell_types <- "exPFC1|exPFC2|exCA1|exCA3|GABA1|GABA2|exDG|ASC1|ASC2|ODC1|ODC2|OPC|MG|NSC|END"
```
...
We apply a k-means method atop the 50 PCs generated earlier, choosing to generate 15 clusters to replicate @habib_massively_2017.
...
As an alternative method, we generate another set of 15 clusters using SC3 [@kiselev_sc3_2017].
We use SC3 to cluster the first 500 cells and then run an SVM to predict the labels of all other cells.
...
all_ks <- 15
sce_hvg <- sc3(
  sce_hvg, gene_filter = FALSE, ks = all_ks, biology = TRUE, rand_seed = 100, svm_num_cells = 500)
...
tmp_lab <- read.table(
  file.path(results_dir, "data", "cluster-num-label.txt"), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)
tmp_lab <- name_change(tmp_lab, "Name", "Cluster.Name")
tmp_lab <- name_change(tmp_lab, "Name.1", "Cell_Type")

tmp_res <- read.table(
  file.path(results_dir, "data", "paper-cluster-res.txt"), sep = "\t", header = TRUE,
...
all_clust_res <- custom_merge(df_hvg, tmp_res)
sc3_res <- data.frame(colData(sce_hvg)[ , paste0("sc3_", all_ks, "_clusters")], stringsAsFactors = FALSE)
for(ks in all_ks) {
...
```{r}
all_vars = c("Cell_Type", paste0("km_", all_num_clust, "_clusters"), paste0("sc3_", all_ks, "_clusters"))
...
```{r}
sc3_plot_consensus(sce, k = 15, show_pdata = c("habib_cluster_name", "cell_id_stem"))
```
...
```{r}
sc3_plot_expression(sce, k = 15, show_pdata = c("habib_cluster_name", "cell_id_stem"))
```
...
```{r}
sc3_plot_de_genes(sce, k = 15, show_pdata = c("habib_cluster_name", "cell_id_stem"))
```
...
```{r}
sc3_plot_markers(sce, k = 15, show_pdata = c("habib_cluster_name", "cell_id_stem"))
```
