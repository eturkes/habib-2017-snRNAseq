...
clust[is.na(clust)] <- "Unknown"
counts <- counts[ , -1]
#keep <- grep("*humanPFCab*", names(counts), value = TRUE) # No idea why I need a "b" in there.
# keep <- grep("*exPFC*", names(clust), value = TRUE)
clust2 <- dplyr::filter(clust, cell_type2 %in% c("exPFC"))
keep <- clust2$sample
counts <- counts[ , keep]
clust <- clust[keep, ]

gene_anno <- readRDS("../results/gene_anno.rds")
gene_keep <- row.names(counts)
gene_anno <- gene_anno[!is.na(gene_anno$hgnc_symbol), ]
gene_anno <- gene_anno[!duplicated(gene_anno$hgnc_symbol), ]
gene_anno <- gene_anno[-1, ]

rownames(gene_anno) <- gene_anno[ , 1]

gene_anno <- gene_anno[gene_keep, ]

#counts2 <- cbind(gene_anno2$ensembl_gene_id, counts)
#data.table::setDT(counts2, keep.rownames = TRUE)[]
#counts3 <- counts2[c(2, 1)]

counts <- cbind(gene_anno$ensembl_gene_id, counts)
counts <- data.table::setDT(counts, keep.rownames = TRUE)[]
tmp <- counts[ , 1]
counts[ , 1] <- counts[ , 2]
counts[ , 2] <- tmp
...
write.table(
  counts,
  file = "../data/GTEx_droncseq_hip_pcf.umi_counts-exPFC.txt",
  quote=FALSE, sep=" ", col.names=FALSE, row.names=FALSE
)
write.table(
  clust,
  file = "../data/GTEx_droncseq_hip_pcf.clusters-exPFC.txt",
