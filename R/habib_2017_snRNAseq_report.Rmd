...
```{r}
counts <- fread(file.path(data_dir, "GTEx_droncseq_hip_pcf.umi_counts.txt.gz"))
...
```{r}
clust <- fread(file.path(data_dir, "GTEx_droncseq_hip_pcf.clusters.txt.gz"))
datatable(clust[1:5, ])
```
...
```{r}
tsne = fread(file.path(data_dir, "../data/GTEx_droncseq_hip_pcf.tsne.txt.gz"))
datatable(tsne[1:5, ])
```
