...
kable(table(colData(sce)$habib_cluster, keep), row.names = TRUE) %>%
  kable_styling(bootstrap_options = "striped", font_size = 10, full_width = FALSE, position = "left")
```
...
It can be seen that the HVG tSNE is an improvement over the tSNE using all genes, so we use this tSNE for the analysis going forward.
...
```{r}
label_df <- data.frame(
  km_17_clusters = levels(df_hvg$km_17_clusters), label = levels(df_hvg$km_17_clusters))
label_df2 <- df_hvg %>%
  group_by(km_17_clusters) %>%
  summarize(habib_tsne1 = median(habib_tsne1), habib_tsne2 = median(habib_tsne2)) %>%
  left_join(label_df)
dim_red_plot(
  data = df_hvg, x = "habib_tsne1", y = "habib_tsne2", col = "km_17_clusters", type = "cat") +
