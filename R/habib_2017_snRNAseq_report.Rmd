...
Let's first define a function for consistent plotting throughout.
...
tmp_lab <- read.table(file.path(assets_dir, "cluster-num-label.txt"), sep = "\t", header = TRUE)
...
tmp_clust <- read.table(
  file.path(assets_dir, "paper-cluster.txt"), sep = "\t", header = TRUE, comment.char = "")
...
# Merge into our data.
all_clust <- merge(df_hvg, tmp_clust, by = intersect(names(df_hvg), names(tmp_clust)))

rm(df_hvg, tmp_clust, tmp_lab)
```
...
for (var in all_vars) {
  print(dim_red_plot(
    data = all_clust, x = "hvg_tsne1", y = "hvg_tsne2", col = var, type = "cat"))}
```
...
for (var in all_vars) {
  print(dim_red_plot(
    data = all_clust, x = "habib_tsne1", y = "habib_tsne2", col = var, type = "cat"))}
```
