...
habib_tsne <- data.frame(reducedDim(sce, "habib_tsne"))
names(habib_tsne) <- paste0("habib_tsne", seq(ncol(habib_tsne)))
df_hvg <- data.frame(
  colData(sce_hvg)[ , c(
...
```{r}
col_dat <- cbind(col_dat, clust[ , names(clust) != "sample"])
sce <- SingleCellExperiment(
  assays = list(counts = counts), colData = col_dat,
  reducedDims = SimpleList(habib_tsne = tsne))
...
df_allgenes <- data.frame(
  colData(sce)[ , c(
    "sample", "cell_id_stem", "log10_total_features_by_counts",
    "habib_cluster", "habib_cluster_name")],
  habib_tsne,
  tmp_df[1:2])
...
rm(Rtsne, pca, tmp_df, habib_tsne)
