...
ribo_genes <- read.table(
  file.path(assets_dir, "cache", "ribo-genes.txt"), sep = "\t",
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "clusters.rds")
if (file.exists(rds)) {
  clusters <- readRDS(rds)
} else {
  clusters <- quickCluster(sce, use.ranks = FALSE, method = "igraph")
  saveRDS(clusters, rds)}
```
...
if (file.exists(rds)) {
  ...
  size_factors <- computeSumFactors(sce, cluster = clusters, min.mean = 0.1)
...
if (file.exists(rds)) {
  ...
  pca <- denoisePCA(sce, technical = new_trend, approx = TRUE)
  saveRDS(pca, rds)}
```
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "tsne.rds")
if (file.exists(rds)) {
  tsne <- readRDS(rds)
} else {
  tsne <- runTSNE(sce, use_dimred = "PCA", perplexity = 30, rand_seed = 100)
  saveRDS(tsne, rds)}
```
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "new_tsne.rds")
if (file.exists(rds)) {
  new_tsne <- readRDS(rds)
} else {
  new_tsne <- Rtsne(pca, pca = FALSE)
  saveRDS(new_tsne, rds)}
```

```{r}
tmp_df <- data.frame(new_tsne$Y, stringsAsFactors = FALSE)
names(tmp_df) <- paste0("hvg_tsne", seq(ncol(tmp_df)))
df_hvg <- data.frame(df_hvg, tmp_df, stringsAsFactors = FALSE)
reducedDims(sce_hvg) <- SimpleList(PCA = pca, TSNE = new_tsne$Y)
rm(new_tsne, pca, tmp_df, sce_hvg)
```
...
We apply a k-means method atop the 50 PCs generated earlier, choosing to generate 15-19 clusters.

```{r}
all_num_clust <- c(15:19)
...
if (file.exists(rds)) {
  ...
  df_redDim <- readRDS(file.path(assets_dir, "cache", "df_redDim_km.rds"))
} else {
  ...
  saveRDS(df_redDim, file.path(assets_dir, "cache", "df_redDim_km.rds"))}
```

## SC3

As an alternative method, we generate another set of 15-19 clusters using SC3 [@kiselev_sc3_2017].
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "sc3_out.rds")
if (file.exists(rds)) {
  sc3_out <- readRDS(rds)
} else {
  sc3_out <- sc3(sce, ks = all_ks, biology = TRUE)
  saveRDS(sc3_out, rds)}
```
...
tmp_lab <- read.table(
  file.path(assets_dir, "cache", "cluster-num-label.txt"), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)
tmp_lab <- name_change(tmp_lab, "Name", "Cluster.Name")
tmp_lab <- name_change(tmp_lab, "Name.1", "Cell_Type")

tmp_clust <- read.table(
  file.path(assets_dir, "cache", "paper-cluster.txt"), sep = "\t", header = TRUE,
  comment.char = "", stringsAsFactors = FALSE)
tmp_clust <- name_change(tmp_clust, "X.Genes", "nGenes")
tmp_clust <- name_change(tmp_clust, "X.Transcripts", "nTranscripts")

tmp_clust <- custom_merge(tmp_clust, tmp_lab[ , c("Cluster.Name", "Cell_Type")], all.x = TRUE)

# Merge into our data.
df_redDim$Cell.ID <- colnames(sce)
df_redDim$hvg_tsne1 <- df_hvg$hvg_tsne1
df_redDim$hvg_tsne2 <- df_hvg$hvg_tsne2
all_clust <- custom_merge(df_redDim, tmp_clust)
tmp_sc3 <- data.frame(colData(sce)[ , paste0("sc3_", all_ks, "_clusters")], stringsAsFactors = FALSE)
for (ks in all_ks) {
  sc3_label <- paste0("sc3_", ks, "_clusters")
  tmp_sc3[ , sc3_label] <- as.factor(tmp_sc3[ , sc3_label])}
tmp_sc3$Cell.ID <- colnames(sce)
all_clust <- custom_merge(all_clust, tmp_sc3)
rm(tmp_sc3, df_redDim, df_hvg, tmp_clust, tmp_lab)
```
...
saveRDS(all_clust, file = file.path(results_dir, "data", "all_clust.rds"))
```
