...
The protocol described in @habib_massively_2017 was followed where possible and included the addition of gene annotations, the removal of low-quality cells, the removal of lowly-expressed genes, and normalization.
...
These objects, along with an SCE object subsetted to the top ~1,000 HVGs (highly variable genes), are included in the `results/data/` directory of this project.
...
Overall, it can be seen that our reproduction and replication of the results in @habib_massively_2017 agree well with the original publication as well as reveal new putative subclusters.
...
gg_df <- data.frame(
  colData(sce)[ , c(
    ...
    "km_17_clusters_name", "km_22_clusters_name", "seurat_clusters_name")])
rownames(gg_df) <- NULL
gg_df$habib_cluster_name <- factor(gg_df$habib_cluster_name)
gg_df$km_17_clusters_name <- factor(gg_df$km_17_clusters_name)
gg_df$km_22_clusters_name <- factor(gg_df$km_22_clusters_name)
...
dim_red_plot(
  ...
  xlab("tSNE 1") + ylab("tSNE 2") + ggtitle("KM 17 Clusters Habib tSNE")
```

### KM 22 Clusters Habib tSNE

```{r, echo = FALSE, message = FALSE}
# Setup for automatic placement of cluster labels.
label_df <- data.frame(
  km_22_clusters_name = levels(gg_df$km_22_clusters_name),
  label = levels(gg_df$km_22_clusters_name))
label_df2 <- gg_df %>%
  group_by(km_22_clusters_name) %>%
  summarize(habib_tsne1 = median(habib_tsne1), habib_tsne2 = median(habib_tsne2)) %>%
  left_join(label_df)

dim_red_plot(
  data = gg_df, x = "habib_tsne1", y = "habib_tsne2", col = "km_22_clusters_name", type = "cat") +
  xlab("tSNE 1") + ylab("tSNE 2") + ggtitle("KM 22 Clusters Habib tSNE")
```
...
### KM 22 Clusters HVG SVD PCA tSNE
...
dim_red_plot(
  data = gg_df, x = "hvg_svd_tsne1", y = "hvg_svd_tsne2", col = "km_22_clusters_name", type = "cat") +
  xlab("tSNE 1") + ylab("tSNE 2") + ggtitle("KM 22 Clusters HVG SVD PCA tSNE")
```

### KM 17 Clusters HVG SVD PCA tSNE
...
dim_red_plot(
  data = gg_df, x = "hvg_svd_tsne1", y = "hvg_svd_tsne2", col = "km_17_clusters_name", type = "cat") +
  xlab("tSNE 1") + ylab("tSNE 2") + ggtitle("KM 17 Clusters HVG SVD PCA tSNE")
```

### Habib Clusters HVG SVD PCA tSNE
...
dim_red_plot(
  ...
  xlab("tSNE 1") + ylab("tSNE 2") + ggtitle("Habib Clusters HVG SVD PCA tSNE")
```
...
In order to standard gene symbols, which is often useful for downstream analysis, we add annotations based on the hg19 reference gnome, the same used in @habib_massively_2017.
...
In a situation were this cutoff was not applied, we might apply it using the `emptyDrops` function from `scater`. which should provide a more robust approximation of empty droplets.

## Additional QC Metadata

When using packages in the SCE ecosytem, it is helpful to apply the `calculateQCMetrics` function from `scater` to generate useful and sometimes necessary metadata.
This function can also assess the proportion of ribosomal/mitochrondrial gene expression in nuclei, which does not appear to have been done in @habib_massively_2017.
This opens doors for additional QC as cells with an abnormally high proportion of these genes are likely to be damaged.
...
hist(
  ...
  breaks = 20, col = "grey80", ylab = "Number of Cells")
hist(
  sce$pct_counts_ribo, xlab = "Ribosomal Proportion Percentage", ylab = "Number of Cells",
  breaks = 40, main = "", col = "grey80")
hist(
  sce$pct_counts_mito, xlab = "Mitochondrial Proportion Percentage", ylab = "Number of Cells",
...
As we intend to replicate the results of @habib_massively_2017, we do not remove any cells detected to have an overabundance of mitochondrial/ribosomal gene expression.
However, we do display what would be removed if we do so, as well as other outlier-based removal, and consider it for downstream analysis.
...
The methods section of @habib_massively_2017 suggests that this has already been done:
...
However, we see that this is not the case our current dataset, either because it had not yet been applied or was applied before cell removal.
...
As this analysis serves to both replicate @habib_massively_2017 and prepare the data for downstream analysis in other projects, we diverge our SCE object into two, with one retaining the full set of genes.

```{r}
orig_sce <- sce
```

In the other, we remove genes in the manner specified in @habib_massively_2017.
...
Apparent is the disproportionately high expression of lncRNAs MALAT1 and MEG3, both of which are known to have higher expression in nuclei, as noted in @habib_massively_2017.
...
This function uses a linear deconvolution system to account for expected variation across different cell/nuclei types/sizes [@l_lun_pooling_2016], producing "scale factor" values that indicate the extent to which a cell/nucleus should be scaled.
...
In an experiment where most systematic differences between cells are driven by capture efficiency and sequencing depth, we should see a correlation between size factors and library size.
...
We therefore add normalized expression data to our SCE object using this method.
...
```{r}
dec1 <- dec
dec1$bio[which(dec$bio < 1e-72)] <- 1.5e-72
...
We use the first 18 PCs, chosen by visually identifying the dropoff in variance explained.
...
In this section, we will see if we can replicate those clusters as well as expand upon putative subclusters such as those seen in the HVG SVD tSNE.
For visualization, we start by overlaying the clusters on the tSNE coordinates in @habib_massively_2017.

## Seurat

We start with Seurat as it heuristically selects the number of clusters rather than the user providing it a-priori.
As input, we use the 16 PCs generated in the HVG Seurat section.

```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "seurat.rds")
if (file.exists(rds)) {
  seurat <- readRDS(rds)
} else {
  seurat <- FindNeighbors(seurat, dims = 1:16)
...
We apply a k-means method atop the 30 SVD PCs and the ~1,000 HVGs in `sce_hvg`.
As Seurat suggested 22 clusters, we do one round with that parameter and another with 17 clusters like in @habib_massively_2017.

```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "km_22.rds")
if (file.exists(rds)) {
  km_22 <- readRDS(rds)
} else {
  km_22 <- kmeans(
    reducedDim(sce_hvg, "PCA"), centers = 22, iter.max = 1e8, nstart = 2500, algorithm = "MacQueen")
  saveRDS(km_22, rds)}
```

```{r}
gg_df$km_22_clusters <- as.factor(km_22$cluster)
rm(km_22)

# Setup for automatic placement of cluster labels.
label_df <- data.frame(
  km_22_clusters = levels(gg_df$km_22_clusters), label = levels(gg_df$km_22_clusters))
label_df2 <- gg_df %>%
  group_by(km_22_clusters) %>%
  summarize(habib_tsne1 = median(habib_tsne1), habib_tsne2 = median(habib_tsne2)) %>%
  left_join(label_df)

dim_red_plot(
  data = gg_df, x = "habib_tsne1", y = "habib_tsne2", col = "km_22_clusters", type = "cat") +
  xlab("tSNE 1") + ylab("tSNE 2") + ggtitle("KM 22 Clusters Habib tSNE")
```

```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "km_17.rds")
if (file.exists(rds)) {
  km_17 <- readRDS(rds)
} else {
  km_17 <- kmeans(
...
We observe that some clustering approaches agree with @habib_massively_2017 better than others, though all show strong overlap in certain clusters.
We assign labels for the new clusters based on their proximity to the clusters in the original paper and then use tSNE to visualize them.

```{r}
gg_df$seurat_clusters_name <- NA
seurat_clusters <- as.integer(gg_df$seurat_clusters)
for (i in 1:length(seurat_clusters)) {
  if (seurat_clusters[i] == 0) {
    gg_df[i, ncol(gg_df)] <- "exPFC1.4"
  } else if (seurat_clusters[i] == 1) {
...
seurat_clusters_id <- c(
  "exPFC1.4", "ODC1.1", "exPFC1.1", "ODC1.2", "exPFC1.2", "ASC1", "GABA1", "exCA3",
  "Unlabeled3", "exDG", "ASC2", "OPC", "GABA2.1", "ODC2", "MG",
  "GABA2.1", "exCA1", "NSC", "exPFC2", "END", "Unlabeled1", "exPFC1.3")
names(seurat_clusters_id) <- levels(seurat)
seurat <- RenameIdents(seurat, seurat_clusters_id)

gg_df$km_22_clusters_name <- NA
km_22_clusters <- as.integer(gg_df$km_22_clusters)
for (i in 1:length(km_22_clusters)) {
  if (km_22_clusters[i] == 1) {
    gg_df[i, ncol(gg_df)] <- "ODC2"
  } else if (km_22_clusters[i] == 2) {
    gg_df[i, ncol(gg_df)] <- "exPFC1.2"
  } else if (km_22_clusters[i] == 3) {
    gg_df[i, ncol(gg_df)] <- "exCA3"
  } else if (km_22_clusters[i] == 4) {
    gg_df[i, ncol(gg_df)] <- "OPC1.2"
  } else if (km_22_clusters[i] == 5) {
    gg_df[i, ncol(gg_df)] <- "GABA2.1"
  } else if (km_22_clusters[i] == 6) {
    gg_df[i, ncol(gg_df)] <- "OPC1.1"
  } else if (km_22_clusters[i] == 7) {
    gg_df[i, ncol(gg_df)] <- "GABA1.2"
  } else if (km_22_clusters[i] == 8) {
    gg_df[i, ncol(gg_df)] <- "exPFC1.1"
  } else if (km_22_clusters[i] == 9) {
    gg_df[i, ncol(gg_df)] <- "ASC2.1"
  } else if (km_22_clusters[i] == 10) {
    gg_df[i, ncol(gg_df)] <- "ASC2.2"
  } else if (km_22_clusters[i] == 11) {
    gg_df[i, ncol(gg_df)] <- "exPFC2"
  } else if (km_22_clusters[i] == 12) {
    gg_df[i, ncol(gg_df)] <- "exDG"
  } else if (km_22_clusters[i] == 13) {
    gg_df[i, ncol(gg_df)] <- "MG"
  } else if (km_22_clusters[i] == 14) {
    gg_df[i, ncol(gg_df)] <- "END"
  } else if (km_22_clusters[i] == 15) {
    gg_df[i, ncol(gg_df)] <- "Unlabeled1"
  } else if (km_22_clusters[i] == 16) {
    gg_df[i, ncol(gg_df)] <- "ODC1.2"
  } else if (km_22_clusters[i] == 17) {
    gg_df[i, ncol(gg_df)] <- "GABA2.2"
  } else if (km_22_clusters[i] == 18) {
    gg_df[i, ncol(gg_df)] <- "NSC"
  } else if (km_22_clusters[i] == 19) {
    gg_df[i, ncol(gg_df)] <- "exCA1"
  } else if (km_22_clusters[i] == 20) {
    gg_df[i, ncol(gg_df)] <- "GABA1.1"
  } else if (km_22_clusters[i] == 21) {
    gg_df[i, ncol(gg_df)] <- "ASC1"
  } else if (km_22_clusters[i] == 22) {
...
for (i in 1:length(km_17_clusters)) {
  if (km_17_clusters[i] == 1) {
    ...
    gg_df[i, ncol(gg_df)] <- "END1.1"
  } else if (km_17_clusters[i] == 17) {
    gg_df[i, ncol(gg_df)] <- "NSC"}}

colData(sce_hvg) <- cbind(
  colData(sce_hvg), seurat_clusters = gg_df$seurat_clusters,
  seurat_clusters_name = gg_df$seurat_clusters_name, km_22_clusters = gg_df$km_22_clusters,
  km_22_clusters_name = gg_df$km_22_clusters_name, km_17_clusters = gg_df$km_17_clusters,
