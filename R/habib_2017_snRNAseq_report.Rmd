...
rds <- file.path(assets_dir, "cache", "highest_exprs.rds")
if (file.exists(rds)) {
  highest_exprs <- readRDS(rds)
} else {
  highest_exprs <- plotHighestExprs(sce)
  saveRDS(highest_exprs, file = rds)}
...
custom_merge = function(x, y, mess = NULL, ...) {
	...
	merge(x, y, by = BiocGenerics::intersect(names(x), names(y)), ...)}
