...
The source code and instructions for rerunning the analysis can be found at [github.com/eturkes/habib-2017-snRNAseq](https://github.com/eturkes/habib-2017-snRNAseq) *(currently private, please email for access)*.
...
dim_red_plot = function(data, x, y, col, type) {
  ...
  if (type == "cont") {
    ...
    gg <- gg + ggplot2::guides(color = ggplot2::guide_legend(override.aes = list(size = 3)))
  }
  gg}

sce <- readRDS(file.path(results_dir, "data", "sce.rds"))
sce_hvg <- readRDS(file.path(results_dir, "data", "sce_hvg.rds"))
df_hvg <- readRDS(file.path(results_dir, "data", "df_hvg.rds"))
```
...
```{r, include = FALSE}
df_hvg <- plyr::rename(
  df_hvg, c("habib_cluster_name" = "habib_clust", "km_17_clusters_name" = "km_clust"))
```
...
plotly::ggplotly(dim_red_plot(
  data = df_hvg, x = "habib_tsne1", y = "habib_tsne2", col = "habib_clust", type = "cat"))
```
...
```{r, echo = FALSE, message = FALSE}
plotly::ggplotly(dim_red_plot(
  data = df_hvg, x = "habib_tsne1", y = "habib_tsne2", col = "km_clust", type = "cat"))
```
...
```{r, echo = FALSE, message = FALSE}
plotly::ggplotly(dim_red_plot(
  data = df_hvg, x = "hvg_tsne1", y = "hvg_tsne2", col = "habib_clust", type = "cat"))
```
...
```{r, echo = FALSE, message = FALSE}
plotly::ggplotly(dim_red_plot(
  data = df_hvg, x = "hvg_tsne1", y = "hvg_tsne2", col = "km_clust", type = "cat"))
```
...
```{r}
packages <- c(
  "conflicted", "BiocFileCache", "data.table", "DT", "SingleCellExperiment", "biomaRt",
  "dplyr", "SummarizedExperiment", "DropletUtils", "scater", "scran",
  "BiocSingular", "ggplot2", "svd", "Rtsne", "S4Vectors")
...
# Also, as this file contained non-standard formatting, minimal manual editing was done for use with R.
...
We then use the `isOutlier` function from `scater` to remove the the mitochondrial/ribosomal cells as well as cells determined to be low quality by their library size and feature counts.
...
table(colData(sce)$habib_cluster, keep)
```

It should be noted that all cells in Cluster 17 are identified as ribosomal genes and thus removed.

```{r}
except_ribo_keep <- !(libsize_drop | feature_drop | mito_drop)
table(colData(sce)$habib_cluster, except_ribo_keep)
```
...
We then remove genes as necessary and view the summary statistics again.
...
To verify that the method is appropriate for our data, we start by plotting each cell's size factor against their total counts.
...
We use the `denoisePCA` from the `scran` package, which automatically selects PCs by modeling technical noise.
...
We first try replicating the tSNE in @habib_massively_2017 using all genes in the set.
Several tSNEs are plotted over a range of perplexity values to find the one that best represents our data.
As the perplexity value chosen in @habib_massively_2017 was 100, we center our range around that.

#### Perplexity = 50

```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "runTSNE_perp50.rds")
if (file.exists(rds)) {
  runTSNE_perp50 <- readRDS(rds)
} else {
  runTSNE_perp50 <- runTSNE(sce, use_dimred = "PCA", perplexity = 50, rand_seed = 100)
  saveRDS(runTSNE_perp50, rds)}
```
...
names(tmp_df) <- paste0("perp50_tsne", 1:2)
df_redDim_perp_50 <- data.frame(df_redDim, tmp_df)
dim_red_plot(
  data = df_redDim_perp_50, x = "perp50_tsne1", y = "perp50_tsne2",
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "runTSNE_perp100.rds")
if (file.exists(rds)) {
  runTSNE_perp100 <- readRDS(rds)
} else {
  runTSNE_perp100 <- runTSNE(sce, use_dimred = "PCA", perplexity = 100, rand_seed = 100)
  saveRDS(runTSNE_perp100, rds)}
```
...
tmp_df <- data.frame(reducedDim(sce, "TSNE"))
rownames(tmp_df) <- NULL
names(tmp_df) <- paste0("perp100_tsne", 1:2)
df_redDim_perp_100 <- data.frame(df_redDim, tmp_df)
dim_red_plot(
  data = df_redDim_perp_100, x = "perp100_tsne1", y = "perp100_tsne2",
  col = "habib_cluster_name", type = "cat")
```
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "runTSNE_perp200.rds")
if (file.exists(rds)) {
  runTSNE_perp200 <- readRDS(rds)
} else {
  runTSNE_perp200 <- runTSNE(sce, use_dimred = "PCA", perplexity = 200, rand_seed = 100)
  saveRDS(runTSNE_perp200, rds)}
```
...
tmp_df <- data.frame(reducedDim(sce, "TSNE"))
rownames(tmp_df) <- NULL
names(tmp_df) <- paste0("perp200_tsne", 1:2)
df_redDim_perp_200 <- data.frame(df_redDim, tmp_df)
dim_red_plot(
  data = df_redDim_perp_200, x = "perp200_tsne1", y = "perp200_tsne2",
  col = "habib_cluster_name", type = "cat")
```

#### Assessment

As the tSNE with perplexity set to 100 appears to have the most similarity with that in @habib_massively_2017, we choose that one going forward.

```{r}
sce <- runTSNE_perp100
tmp_df <- data.frame(reducedDim(sce, "TSNE"))
rownames(tmp_df) <- NULL
names(tmp_df) <- paste0("allgenes_tsne", 1:2)
df_redDim <- data.frame(df_redDim, tmp_df)
rm(
  runTSNE_perp200, runTSNE_perp100, runTSNE_perp50,
  df_redDim_perp_200, df_redDim_perp_100, df_redDim_perp_50)

dim_red_plot(
  data = df_redDim, x = "allgenes_tsne1", y = "allgenes_tsne2",
  col = "habib_cluster_name", type = "cat")
dim_red_plot(
  data = df_redDim, x = "allgenes_tsne1", y = "allgenes_tsne2", col = "cell_id_stem", type = "cat")
dim_red_plot(
  data = df_redDim, x = "allgenes_tsne1", y = "allgenes_tsne2",
  col = "log10_total_features_by_counts", type = "cont")
```
...
table(dec$FDR < 1e-10, dec$bio > 0.015)
par(mfrow = c(1, 2))
...
```{r}
keep <- which(dec$FDR < 1e-10 & dec$bio > 0.02)
sce_hvg <- sce[keep, ]
...
dim_red_plot(
  data = df_hvg, x = "hvg_pc1", y = "hvg_pc2",
  col = "log10_total_features_by_counts", type = "cont")
```

### tSNE

Using our HVG PCs we again plot tSNEs over a range of perplexity values.

#### Perplexity = 50

```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "Rtsne_perp50.rds")
if (file.exists(rds)) {
  Rtsne_perp50 <- readRDS(rds)
} else {
  set.seed(100)
  Rtsne_perp50 <- Rtsne(pca, pca = FALSE, perplexity = 50)
  saveRDS(Rtsne_perp50, rds)}
```

```{r}
tmp_df <- data.frame(Rtsne_perp50$Y)
names(tmp_df) <- paste0("perp50_tsne", seq(ncol(tmp_df)))
df_hvg_perp50 <- data.frame(df_hvg, tmp_df)
dim_red_plot(
  data = df_hvg_perp50, x = "perp50_tsne1", y = "perp50_tsne2",
  col = "habib_cluster_name", type = "cat")
```
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "Rtsne_perp100.rds")
if (file.exists(rds)) {
  Rtsne_perp100 <- readRDS(rds)
} else {
  set.seed(100)
  Rtsne_perp100 <- Rtsne(pca, pca = FALSE, perplexity = 100)
  saveRDS(Rtsne_perp100, rds)}
```

```{r}
tmp_df <- data.frame(Rtsne_perp100$Y)
names(tmp_df) <- paste0("perp100_tsne", seq(ncol(tmp_df)))
df_hvg_perp100 <- data.frame(df_hvg, tmp_df)
dim_red_plot(
  data = df_hvg_perp100, x = "perp100_tsne1", y = "perp100_tsne2",
  col = "habib_cluster_name", type = "cat")
```
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "Rtsne_perp200.rds")
if (file.exists(rds)) {
  Rtsne_perp200 <- readRDS(rds)
} else {
  set.seed(100)
  Rtsne_perp200 <- Rtsne(pca, pca = FALSE, perplexity = 200)
  saveRDS(Rtsne_perp200, rds)}
```

```{r}
tmp_df <- data.frame(Rtsne_perp200$Y)
names(tmp_df) <- paste0("perp200_tsne", seq(ncol(tmp_df)))
df_hvg_perp200 <- data.frame(df_hvg, tmp_df)
dim_red_plot(
  data = df_hvg_perp200, x = "perp200_tsne1", y = "perp200_tsne2",
  col = "habib_cluster_name", type = "cat")
```

#### Assessment

Like the tSNE with all genes, we see that a perplexity value of 100 agrees best with @habib_massively_2017.
It is also an improvement over the tSNEs using all genes, so we use this HVG tSNE for the rest of the analysis going forward.

```{r}
tmp_df <- data.frame(Rtsne_perp100$Y)
names(tmp_df) <- paste0("hvg_tsne", seq(ncol(tmp_df)))
df_hvg <- data.frame(df_hvg, tmp_df)
reducedDims(sce_hvg) <- SimpleList(PCA = pca, TSNE = Rtsne_perp100$Y)
rm(
  Rtsne_perp200, Rtsne_perp100, Rtsne_perp50, df_hvg_perp200,
  df_hvg_perp100, df_hvg_perp50, pca, tmp_df)
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "df_hvg.rds")
if (file.exists(rds)) {
  df_hvg <- readRDS(rds)
} else {
  for (i in num_clust) {
    kmeans_out <- kmeans(
      reducedDim(sce_hvg, "PCA"), centers = i, iter.max = 1e8,
      nstart = 2500, algorithm = "MacQueen")
    km_label <- paste0("km_", i, "_clusters")
...
```{r}
vars <- c("habib_cluster_name", paste0("km_", num_clust, "_clusters"))
for (i in vars) {
  print(dim_red_plot(
    data = df_hvg, x = "habib_tsne1", y = "habib_tsne2", col = i, type = "cat"))}
```
...
```{r}
df_hvg$km_17_clusters_name <- NA
km_17_clusters <- as.integer(df_hvg$km_17_clusters)
for (i in 1:length(km_17_clusters)) {
  if (km_17_clusters[i] == 1) {
    df_hvg[i, ncol(df_hvg)] <- "NSC"
  } else if (km_17_clusters[i] == 2) {
    df_hvg[i, ncol(df_hvg)] <- "OPC1"
  } else if (km_17_clusters[i] == 3) {
    df_hvg[i, ncol(df_hvg)] <- "END"
  } else if (km_17_clusters[i] == 4) {
    df_hvg[i, ncol(df_hvg)] <- "GABA2"
  } else if (km_17_clusters[i] == 5) {
    df_hvg[i, ncol(df_hvg)] <- "ODC2"
  } else if (km_17_clusters[i] == 6) {
    df_hvg[i, ncol(df_hvg)] <- "exCA1"
  } else if (km_17_clusters[i] == 7) {
    df_hvg[i, ncol(df_hvg)] <- "MG"
  } else if (km_17_clusters[i] == 8) {
    df_hvg[i, ncol(df_hvg)] <- "ASC3"
  } else if (km_17_clusters[i] == 9) {
    df_hvg[i, ncol(df_hvg)] <- "exDG"
  } else if (km_17_clusters[i] == 10) {
    df_hvg[i, ncol(df_hvg)] <- "GABA1"
  } else if (km_17_clusters[i] == 11) {
    df_hvg[i, ncol(df_hvg)] <- "ODC3"
  } else if (km_17_clusters[i] == 12) {
    df_hvg[i, ncol(df_hvg)] <- "ASC1"
  } else if (km_17_clusters[i] == 13) {
    df_hvg[i, ncol(df_hvg)] <- "exPFC"
  } else if (km_17_clusters[i] == 14) {
    df_hvg[i, ncol(df_hvg)] <- "OPC2"
  } else if (km_17_clusters[i] == 15) {
    df_hvg[i, ncol(df_hvg)] <- "ASC2"
  } else if (km_17_clusters[i] == 16) {
    df_hvg[i, ncol(df_hvg)] <- "ODC1"
  } else if (km_17_clusters[i] == 17) {
    df_hvg[i, ncol(df_hvg)] <- "exCA3"}}
```
...
```{r}
vars <- c("habib_cluster_name", paste0("km_", num_clust, "_clusters_name"))
```

### Habib tSNE

Using the tSNE coordinates in @habib_massively_2017.
...
Using the new tSNE based on HVGs.

```{r}
for (i in vars) {
  print(dim_red_plot(
    data = df_hvg, x = "hvg_tsne1", y = "hvg_tsne2", col = i, type = "cat"))}
```
...
saveRDS(df_hvg, file.path(results_dir, "data", "df_hvg.rds"))
