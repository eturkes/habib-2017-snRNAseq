...
The processed data is included in this analysis as an SCE (SingleCellExperiment) object alongside an SCE subsetted to the top HVGs (highly variable genes, about 2,000) and an SCE without additional QC.
They can be found in the `results/data/` directory.
...
```{r, include = FALSE}
rm(gg_df, label_df, label_df2, sce, sce_hvg, sce_orig, seurat)
```
...
In those cases we double-check that they were done carry them out if not.
...
As this step is destructive, we first save a copy of the SCE object thus far for analyses that wish to use the complete data.
...
We plot the coordinates provided in @habib_massively_2017, which were generated off of select PCs after subsetting to the most highly variable genes.
...
To replicate @habib_massively_2017, we start by applying `denoisePCA` from the `scran` package.
This function can automatically select the number of PCs to retain through the modeling of technical noise.
As @habib_massively_2017 describes, PC selection was performed but we do not know the exact number retained:
...
Although we only intend to use `denoisePCA` for PC selection, we see what a tSNE would look like with it as input.
We use a perplexity of 100 for all tSNE plots, the same value used in @habib_massively_2017.
...
if (file.exists(rds)) {
  ...
  tsne <- runTSNE(
    sce, use_dimred = "PCA", n_dimred = ncol(reducedDim(sce, "PCA")), perplexity = 100, rand_seed = 1)
  saveRDS(tsne, rds)}
```
...
While the tSNE has fair agreement with @habib_massively_2017, it shows decreased ability to resolve clusters and is strongly influenced by a batch effects, particularly from certain hippocampal samples.

### SVD PCA

The PCA method used for downstream analysis in @habib_massively_2017 is described:

> We used the fast ‘rpca’ function in R (package ‘rsvd’)...

We use it as well, with a number of PCs equal to those suggested by `denoisePCA`.
...
if (file.exists(rds)) {
  ...
  set.seed(1)
  rpca <- rpca(edat, k = ncol(reducedDim(sce, "PCA")))
  saveRDS(rpca, rds)}
```

```{r}
add_df <- data.frame(rpca$x)
...
if (file.exists(rds)) {
  ...
  set.seed(1)
  tsne <- Rtsne(rpca$x, pca = FALSE, perplexity = 100)
  saveRDS(tsne, rds)}
```

```{r}
reducedDims(sce) <- SimpleList(PCA = rpca$x, TSNE = tsne$Y)
add_df <- data.frame(reducedDim(sce, "TSNE"))
...
We see that the batch effect is somewhat mitigated and the overall visualization is greatly improved.
...
The number of HVGs is not stated in @habib_massively_2017, so we threshold FDR and biological component values to select a conventional amount of roughly 2,000 HVGs and create a new SCE object.

```{r, fig.height = 6}
dec_hist <- dec
dec_hist$FDR[which(dec$FDR < 0.001)] <- 0.001
dec_hist$bio[which(dec$bio >= 0.008)] = 0.008
par(mfrow = c(1, 2))
hist(log10(dec_hist$FDR), breaks = 100, main = "")
hist(log10(dec_hist$bio), breaks = 100, main = "")

keep <- which(dec$FDR < 0.001 & dec$bio >= 0.008)
sce_hvg <- sce[keep, ]
rm(dec, dec_hist)
sce_hvg
```
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "quickCluster2.rds")
if (file.exists(rds)) {
  quickCluster <- readRDS(rds)
} else {
  set.seed(1)
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "size_factors2.rds")
if (file.exists(rds)) {
  size_factors <- readRDS(rds)
} else {
...
```{r}
ncol(reducedDim(sce_hvg, "PCA"))
par(mfrow = c(1, 1))
plot(
  log10(attr(reducedDim(sce_hvg), "percentVar")), xlab = "PC",
  ylab = "log10(Proportion of Variance Explained)", pch = 20,
  cex = 0.6, col = rgb(0.8, 0.2, 0.2, 0.5))
abline(v = ncol(reducedDim(sce_hvg, "PCA")), lty = 2, col = "red")
```
...
if (file.exists(rds)) {
  ...
  set.seed(1)
  rpca <- rpca(edat, k = ncol(reducedDim(sce_hvg, "PCA")))
  saveRDS(rpca, rds)}
```
...
```{r}
reducedDims(sce_hvg) <- SimpleList(PCA = rpca$x, TSNE = tsne$Y)
...
After following the methods of @habib_massively_2017 closely, we find that the resulting dimensionality reduction best captures the one seen in the original publication.
...
The authors explain that they computed clusters by:

> using the top significant PCs as an input to a graph-based clustering algorithm...

## Seurat

The package `Seurat` uses a nearest neighbor graph clustering method, like that in @habib_massively_2017.
As part of our input, we use the genes and SVD PCs selected during the HVG steps.
The `resolution` parameter of `FindClusters()` is tweaked to generate the desired 18 clusters.

```{r}
seurat <- as.Seurat(sce_hvg, counts = "counts", data = NULL)
seurat <- NormalizeData(seurat)
seurat <- FindVariableFeatures(seurat, nfeatures = dim(sce_hvg)[1])
seurat <- ScaleData(seurat, features = rownames(seurat))
seurat <- FindNeighbors(seurat, reduction = "PCA", dims = 1:ncol(reducedDim(sce_hvg, "PCA")))
...
Our results show interesting similarities and differences with @habib_massively_2017.
One interpretation is that our attempt clustered common cell types (GABA, ASC, ODC, exCA) more broadly while pulling out a few new putative rare cell types.
While this pattern is not fully represented by the tSNE coordinates from @habib_massively_2017, we see that they are actually well supported by our tSNE that used the same inputs as to Seurat.
...
dim_red_plot(
  data = gg_df, x = "hvg_svd_tsne1", y = "hvg_svd_tsne2", col = "seurat_clusters", type = "cat") +
  xlab("tSNE 1") + ylab("tSNE 2") + ggtitle("Seurat Clusters HVG SVD tSNE")
```

We conclude that our replication was generally successful but with the potential for improvement as well as findings that may be complementary to @habib_massively_2017.

## Labeling {.tabset}

Finally, we assign labels for the new clusters based on their proximity to the clusters in the original paper and visualize them using tSNE.
