...
![](../assets/reference/habib-fig2a.png)
...
![](../assets/reference/habib-fig2d.png)
...
```{r, echo = FALSE, message = FALSE}
# Use full gene list in case some were lost during HVG step.
...
![](../assets/reference/habib-fig2b.png)
...
clust <- fread(file.path(assets_dir, "reference", "nmeth.4407-S10-edited.txt"), data.table = FALSE)
datatable(clust[1:5, ])
```
...
We also set aside an unprocessed copy of the object to be saved for later.
...
names(rowData(sce))[names(rowData(sce)) == "strand"] <- "strand_n" # Avoids package conflictions.
...
We start by investigating low library and feature sizes, as well ribosomal/mitochondrial gene expression, as these were measured in the publication but it is unclear whether the data was filtered on that basis.
Mitochondrial genes can be extracted simply by their prefix, while an annotation set was downloaded from HUGO for ribosomal genes.

```{r}
ribo_genes <- read.table(file.path(getwd(), "..", "hugo-ribo-genes.txt"), sep = "\t", header = TRUE)
...
if (file.exists(rds)) {
  ...
  set.seed(1)
  quickCluster <- quickCluster(sce, use.ranks = FALSE, BPPARAM = MulticoreParam())
  saveRDS(quickCluster, rds)}
```

```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "computeSumFactors.rds")
if (file.exists(rds)) {
  computeSumFactors <- readRDS(rds)
} else {
  computeSumFactors <- computeSumFactors(
    sce, cluster = quickCluster, min.mean = 0.1, BPPARAM = MulticoreParam())
  saveRDS(computeSumFactors, rds)}
```
...
>  To rule out the possibility that the resulting clusters are driven by batch or other technical effects, we examined the distribution of samples within each cluster and the distribution of the number of genes detected across clusters (as a measure of nuclei quality).
Overall, the nuclei separated into distinct point clouds in tSNE space that were not driven by batch; each cluster or cloud was an admixture of cells from all technical and biological replicates, with variable numbers of genes.
Related to the number of genes, we note that there is a distinct biological difference in cell size (and expected RNA content) between neuronal and glial cells in the brain.

To assess these qualities we use the tSNE coordinates and color each point by the batch they were derived and by their total features.
...
Although the dataset is not perfectly homogeneous, we continue forward as no correction was applied in the papers, noting that this may be valuable to do for re-analysis.
We also see that the cluster labeled plot is identical to Figure 2a from the paper, aside from the additional "Unlabeled" cells.

# Dimensionality Reduction

In generating their tSNE coordinates, as well as clusters, the authors used select principal components after subsetting to the most highly variable genes.
...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "makeTechTrend.rds")
if (file.exists(rds)) {
  makeTechTrend <- readRDS(rds)
} else {
  makeTechTrend <- makeTechTrend(x = sce, MulticoreParam())
  saveRDS(makeTechTrend, rds)}
```
...
curve(makeTechTrend(x), col = "red", lwd = 2, add = TRUE)
legend(
...
sce_hvg <- sce[keep, ]
metadata(sce) <- list(hvg = rownames(sce_hvg))
rm(dec)
dim(sce_hvg)[1]
```
...
if (file.exists(rds)) {
  ...
  quickCluster <- quickCluster(sce_hvg, use.ranks = FALSE, BPPARAM = MulticoreParam())
  saveRDS(quickCluster, rds)}
```

```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "hvg_computeSumFactors.rds")
if (file.exists(rds)) {
  computeSumFactors <- readRDS(rds)
} else {
  computeSumFactors <- computeSumFactors(
    sce_hvg, cluster = quickCluster, min.mean = 0.1, BPPARAM = MulticoreParam())
  saveRDS(computeSumFactors, rds)}
```

```{r}
sce_hvg <- computeSumFactors
sce_hvg <- normalize(sce_hvg)
rm(computeSumFactors)
sce_hvg
```
...
We perform PCA in the manner described in @habib_massively_2017:
...
plotPCA(sce_hvg, ncomponents = 4, colour_by = "cell_id_stem") +
  ggtitle("Sample Distribution HVG PCA") + theme(legend.title = element_blank())
plotPCA(sce_hvg, ncomponents = 4, colour_by = "log10_total_features_by_counts") +
  ggtitle("log10(Total Features) HVG PCA") + theme(legend.title = element_blank())
```
...
Taking the explanation literally, the using largest eigen value gap would retain very few PCs, as can be seen on the below scree plot.
Instead, we choose a cutoff point just past the elbow where eigen values begin to stabilize.

```{r}
retain_pcs <- 22
retain_pcs
ggscreeplot(pca, type = "eigenvals") +
  geom_line(linetype = "solid") +
  geom_vline(xintercept = retain_pcs, color = "#EF3B2C", linetype = "dashed")
datatable_custom(summary(pca))
```
...
if (file.exists(rds)) {
  ...
  set.seed(1)
  tsne <- Rtsne(pca$x[ , 1:retain_pcs], pca = FALSE, perplexity = 100, max_iter = 2000)
  saveRDS(tsne, rds)}
```
...
# Setup for automatic placement of cluster labels.
label_df <- data.frame(
  seurat_hvg_clusters = levels(gg_df$seurat_hvg_clusters),
  label = levels(gg_df$seurat_hvg_clusters))
label_df2 <- gg_df %>%
