...
```{r, cache = TRUE}
rds <- file.path(assets_dir, "cache", "kmeans_out.rds")
if (file.exists(rds)) {
  kmeans_out <- readRDS(rds)
  km_label <- readRDS(file.path(assets_dir, "cache", "km_label.rds"))
  ...
  saveRDS(kmeans_out, rds)
  saveRDS(km_label, file.path(assets_dir, "cache", "km_label.rds"))
...
# Hybrid SVM approach
# sce <- sc3(
#   sce, gene_filter = FALSE, ks = all_ks, biology = TRUE, rand_seed = 100, svm_num_cells = 2000)
...
tmp_res <- read.table(
  file.path(assets_dir, "cache", "paper-cluster-res.txt"), sep = "\t", header = TRUE,
...
Genes with $AUROC > 0.85$ and $p < 0.01$ are selected and the top 10 are visualized in the following figure.

```{r}
for (k in all_ks) {
  sc3_plot_markers(sce, k = k, show_pdata = p_data)}
```
...
saveRDS(sce_hvg, file = file.path(results_dir, "data", "sce_hvg.rds"))
saveRDS(all_clust_res, file = file.path(results_dir, "data", "all_clust_res.rds"))
```
