...
counts <- data.table::fread(
    cmd = sprintf(
        "zcat < %s",
        file.path(getwd(), "../data/GTEx_droncseq_hip_pcf.umi_counts.txt.gz")
    ),
    data.table = FALSE
)
...
col_dat <- data.frame(sample <- colnames(counts),
...
clust <- data.table::fread(
    cmd <- sprintf(
        "zcat < %s", file.path(getwd(), "../data/GTEx_droncseq_hip_pcf.clusters.txt.gz")
    ),
    data.table = FALSE
)
